...
class SendAppointmentConfirmedNotification
{
    public function handle(AppointmentConfirmed $event): void
    {
        ...
        if (!$patientEmail) {
            Log::warning('Cannot send appointment confirmation email: patient email not found', [
        ...
        try {
            ...
            if (!$mailSettings) {
                Log::warning('Mail settings not configured in database for appointment confirmation');
                return;
            }
            ...
            Log::info('Sending appointment confirmed email', [
                'appointment_id' => $appointment->id,
                'patient_email' => $patientEmail,
                'appointment_type' => $appointment->appointmentType?->name ?? $appointment->type,
                'mail_from' => $mailSettings->email,
            ]);

            Mail::to($patientEmail)->send(new AppointmentConfirmedMail($appointment));

            Log::info('Appointment confirmed email sent successfully', [
                'appointment_id' => $appointment->id,
                'patient_email' => $patientEmail,
            ]);
        } catch (\Exception $e) {
