...
use App\Mail\MedCertRejectedMail;
use App\Models\MailSetting;
use Illuminate\Support\Facades\Mail;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Crypt;

class SendMedCertRejectedNotification
{
    public function handle(MedCertRejected $event): void
    {
        Log::info('MedCertRejected event triggered', ['medcert_id' => $event->medCert->id]);
        
        try {
            $medCert = $event->medCert->load(['patient.user']);
            $patientEmail = $medCert->patient?->user?->email;

            Log::info('Processing medical certificate rejection', [
                'medcert_id' => $medCert->id,
                'patient_email' => $patientEmail,
            ]);

            if (!$patientEmail) {
                Log::warning('MedCert rejected but patient email not found', ['medcert_id' => $medCert->id]);
                return;
            }
            ...
            // Send the email
            Mail::to($patientEmail)->send(new MedCertRejectedMail($medCert));

            Log::info('Medical certificate rejected email sent successfully', [
                'medcert_id' => $medCert->id,
                'patient_email' => $patientEmail,
            ]);
        } catch (\Exception $e) {
            Log::error('Failed to send medical certificate rejected email', [
                'error' => $e->getMessage(),
